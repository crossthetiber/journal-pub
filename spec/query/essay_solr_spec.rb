...
describe EssaySolr do
  ...
  it 'has facets' do
    ...
    expect_any_instance_of(Sunspot::DSL::Search).to receive(:facet).with(:award_title)
    subject.search
  end
  ...
  it 'excludes award_title filter' do
    subject.stub(:params).and_return({award_title: 'Snite Museum of Art'})
    expect_any_instance_of(Sunspot::DSL::Search).to receive(:facet).with(:essay_style_title)
    expect_any_instance_of(Sunspot::DSL::Search).to receive(:facet).with(:issue_year)
    expect_any_instance_of(Sunspot::DSL::Search).to receive(:facet).with(:medium)
    expect_any_instance_of(Sunspot::DSL::Search).to receive(:facet).with(:award_title, exclude: [kind_of(Sunspot::Query::Restriction::EqualTo)])
    subject.search
  end
